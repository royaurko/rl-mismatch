...
def get_dict(num_features, num_bins):
...
def q_learning(environment, robust, p_env, p_est, qtable=None, update=True, num_episode=1000, fname=None):
    env = gym.make(environment)
    experiment_filename = environment + '-experiment'
    ...
    num_action_bins = 5
    num_state_bins = 5
    num_action_features = env.action_space.shape[0]
    num_state_features = env.observation_space.shape[0]

    bin0 = pd.cut([-2.4, 2.4], bins=num_state_bins, retbins=True)[1][1:-1]
    bin1 = pd.cut([-2, 2], bins=num_state_bins, retbins=True)[1][1:-1]
    bin2 = pd.cut([-1, 1], bins=num_state_bins, retbins=True)[1][1:-1]
    bin3 = pd.cut([-3.5, 3.5], bins=num_state_bins, retbins=True)[1][1:-1]
    state_bins = [bin0, bin1, bin2, bin3]

    last_time_steps = np.ndarray(0)
    action_bins = pd.cut([-5, 5], bins=num_action_bins, retbins=True)[1][1:-1]
    action_bin_int = pd.cut([-5, 5], bins=num_action_bins, retbins=True)[1]
    ...
    learner = QLearner(num_states=num_state_bins ** num_state_features,
                       num_actions=num_action_bins,
                       alpha=0.2,
                       gamma=1,
                       random_action_rate=0.5,
                       random_action_decay_rate=0.99)
    ...
    history = []
    ave_cumu_r = None
    state_dict = get_dict(num_state_features, num_state_bins)
    action_dict = get_dict(num_action_features, num_action_bins)
    for episode in xrange(num_episode):
        observation = env.reset()
        state_key = tuple(to_bin(observation[i], state_bins[i])
                          for i in xrange(len(observation)))
        state = state_dict[state_key]
        action_idx = learner.set_initial_state(state)
        action = np.random.uniform(action_bin_int[action_idx - 1], action_bin_int[action_idx])
        ...
        for step in xrange(max_number_of_steps - 1):
            ...
            if rand > threshold:
                state_prime = random.choice(state_dict.values())
            else:
                state_prime = state_dict[state_key]

            if done:
                reward = -200

            cumu_r = reward + learner.gamma * cumu_r
            action_idx = learner.move(state_prime, reward, robust, p_est, update)
            action = np.random.uniform(action_bin_int[action_idx - 1], action_bin_int[action_idx])
